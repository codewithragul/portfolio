...
// // public/body.js (debug version)
// // This will try to POST to your Express server and log everything clearly.

// (function(){
//   const ENDPOINT = 'http://localhost:3000/api/messages'; // must match running server
//   const emailPattern = /^[A-Za-z0-9]+@[A-Za-z0-9]+\.[A-Za-z]{2,}$/;

//   // small helper: use existing showPopup if present, otherwise fallback to alert
//   function showPopupMaybe(type, title, message) {
//     if (typeof showPopup === 'function') {
//       try { showPopup(type, title, message, 'OK', 2500); return; } catch(e) {}
//     }
//     alert(title + '\n\n' + message);
//   }

//   function log(msg, ...args) { console.log('[contact-debug]', msg, ...args); }

//   function init(){
//     const form = document.querySelector('.contact-form');
//     if (!form) return log('No .contact-form found on page');
...
//       log('Form values:', { fullName, email, phone, subject, message });

//       // client validation
//       if (!fullName || !email || !message) {
//         showPopupMaybe('error','Missing fields','Name, email and message are required.');
//         return;
//       }
//       if (!emailPattern.test(email)) {
//         showPopupMaybe('error','Invalid email','Please enter a valid email (letters/numbers only).');
//         return;
//       }

//       const payload = { fullName, email, phone, subject, message, submittedAt: new Date().toISOString(), page: location.href };
//       log('Sending payload to', ENDPOINT, payload);
...
//         log('Fetch returned status', resp.status);
...
//         if (resp.ok) {
//           showPopupMaybe('success','Message sent','Message saved successfully on server.');
//           form.reset();
//           log('Success — message saved. Check messages.json on server.');
//         } else {
//           // server-level error (400/500). Show server's error message if any.
//           const serverMsg = (json && json.error) ? json.error : text || ('Status ' + resp.status);
//           showPopupMaybe('error','Server error', serverMsg);
//           log('Server error:', resp.status, serverMsg);
//         }
//       } catch (err) {
//         // network error / CORS / server down
//         console.error('[contact-debug] Network/Fetch error:', err);
//         showPopupMaybe('error','Network error','Could not reach server: ' + err.message);
//       }
//     });
//   }
...
// =================================================================== //
// public/body.js (navigation + contact handling)
// Put this file in public/ (or overwrite your existing body.js that's loaded by index.html)

///// NAV / UI CODE (your existing navigation/resume/project code) /////
// const navs = document.querySelectorAll('.nav-list li');
// const cube = document.querySelector('.box');
// const sections = document.querySelectorAll('.section');
...
// setTimeout(() => {
//     if (sections[4]) sections[4].classList.remove('active');
// }, 1500);

// public/body.js - navigation + modern popup + contact handler

/* ========================= NAV / UI CODE (unchanged) ======================= */
// const navs = document.querySelectorAll('.nav-list li');
// const cube = document.querySelector('.box');
// const sections = document.querySelectorAll('.section');
...
//     cursor: pointer;
//     color: #fff;
//     background: linear-gradient(90deg,#00c37a,#00a6ff);
//   }
...
// (function () {
//   // choose endpoint:
//   // If you're serving site from Express (recommended) then relative '/api/messages' works.
//   // If you're running Live Server at port 5500, we point to localhost:3000 explicitly.
//   const ENDPOINT = (window.location.port === '5500')
...
//           // ⭐ KEEP CONTACT PAGE OPEN (this prevents jumping to home)
//           sections.forEach(s => s.classList.remove('active'));
//           sections[4].classList.add('active');
...
// =================================================================== //// server.js - Express server to serve static files and handle contact form submissions

/* debug-body.js (replace public/body.js with this) */
/* NAV + UI (unchanged) */
...
// === Removed automatic hiding of contact section to prevent unexpected jumps ===
// setTimeout(() => {
//   if (sections[4]) sections[4].classList.remove('active');
// }, 1500);

/* Modern popup UI (updated to set popupOpen flag) */
(function () {
  const css = `
